...
bool Bird::hit(Pipe *pipe){
	...
	int pipe_center_y = pipe_y + pipe_h / 2; //lay phan giua cua 2 ong
	if((m_position.getX()  + m_width > pipe_x) && (m_position.getX() < pipe_x + pipe_w)){
		if((m_position.getY() + m_height < pipe_center_y + pipe->getGap() / 2 )&& (m_position.getY() > pipe_center_y - pipe->getGap() / 2))//tao khoang trong o phan giua 2 ong
        {
